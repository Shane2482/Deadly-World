...
public class block_lamps_extras extends Block implements IMetaBlockName {

	public static final PropertyEnum TYPE = PropertyEnum.create("type", lampType2.class);

	public final boolean isOn;

	public block_lamps_extras(Material material, String name, String regName, float hardness, float resistance, String tool,	int level, boolean isOn) {
		super(material);
		...
		setDefaultState(blockState.getBaseState().withProperty(TYPE, lampType2.Yellow));
		this.isOn = isOn;
	...
	protected void registerRendering() {
		for (int i = 0; i < lampType2.values().length; i++) {
			DeadlyWorld.proxy.addRenderRegister(new ItemStack(this, 1, i), getRegistryName(),
					TYPE.getName() + "=" + lampType2.values()[i].name);
		}
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote && !player.isSneaking()) {
			world.setBlockState(pos,
					(this.isOn ? ModBlocks.LampsExtras : ModBlocks.LampsOnExtras).getStateFromMeta(this.getMetaFromState(state)), 2);
	...
	private void updateLamps(World world, BlockPos pos, boolean powered, List<BlockPos> list) {
		...
		if (block instanceof block_lamps_extras) {
			boolean isOn = ((block_lamps_extras) block).isOn;
	...
	public int getMetaFromState(IBlockState state) {
		lampType2 type = (lampType2) state.getValue(TYPE);
		return type.ID;
	}

	@Override
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(TYPE, lampType2.values()[meta]);
	}
	...
	public String getSpecialName(ItemStack stack) {
		return lampType2.values()[stack.getItemDamage()].getName();
	}

	// Enum
	
	public static enum lampType2 implements IStringSerializable {
		...
		private lampType2(int ID, String name) {
			this.name = name;
			this.ID = ID;
		}
